...
.directive("expandablePathToggle",['$mdDialog', function($mdDialog){
  return{
    ...
    link: function($scope, element) {
      ...
      console.log("preview path in element state outline directive :   "+JSON.stringify($scope.path));
      for(var i=0; i < $scope.path.length; i++){
        console.log("preview path in element state outline directive :   "+JSON.stringify($scope.path[i]));
